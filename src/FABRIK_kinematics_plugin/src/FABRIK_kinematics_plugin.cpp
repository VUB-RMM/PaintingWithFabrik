...
#include <FABRIK_kinematics_plugin/FABRIK_kinematics_plugin.h>
...
{
FABRIKKinematicsPlugin::FABRIKKinematicsPlugin() : active_(false)
{
}
                                     ...
                                     double search_discretization)
{
  ...
  ROS_INFO_STREAM_NAMED("FABRIK", "FABRIKKinematicsPlugin initializing");
  ...
  ROS_INFO_STREAM_NAMED("FABRIK", "Dimension planning group '"
  ...
  ROS_DEBUG_STREAM_NAMED("FABRIK", "Looking for ROS service name on rosparam server with param: "
  ...
  if (!ik_service_client_->waitForExistence(ros::Duration(0.1)))  // wait 0.1 seconds, blocking
    ROS_WARN_STREAM_NAMED("FABRIK",
                          "Unable to connect to ROS service client with name: " << ik_service_client_->getService());
  else
    ROS_INFO_STREAM_NAMED("FABRIK", "Service client started with ROS service name: " << ik_service_client_->getService());

  active_ = true;
  ROS_DEBUG_NAMED("FABRIK", "ROS service-based kinematics solver initialized");
  return true;
}

bool FABRIKKinematicsPlugin::setRedundantJoints(const std::vector<unsigned int>& redundant_joints)
{
  if (num_possible_redundant_joints_ < 0)
  {
    ROS_ERROR_NAMED("FABRIK", "This group cannot have redundant joints");
    return false;
  }
  if (int(redundant_joints.size()) > num_possible_redundant_joints_)
  {
    ROS_ERROR_NAMED("FABRIK", "This group can only have %d redundant joints", num_possible_redundant_joints_);
    return false;
  }
...
bool FABRIKKinematicsPlugin::isRedundantJoint(unsigned int index) const
{
...
int FABRIKKinematicsPlugin::getJointIndex(const std::string& name) const
{
...
bool FABRIKKinematicsPlugin::searchPositionIK(const std::vector<geometry_msgs::Pose>& ik_poses,
                                           ...
                                           const moveit::core::RobotState* /*context_state*/) const
{
  ...
  if (!active_)
  {
    ROS_ERROR_NAMED("FABRIK", "kinematics not active");
  ...
  {
    ROS_ERROR_STREAM_NAMED("FABRIK", "Mismatched number of pose requests (" << ik_poses.size() << ") to tip frames ("
                                                                         << tip_frames_.size()
                                                                         << ") in searchPositionIK");
    error_code.val = error_code.NO_IK_SOLUTION;
    return false;
  ...
  if (ik_service_client_->call(ik_srv))
  {
    ...
    {
      ROS_DEBUG_STREAM_NAMED("FABRIK", "An IK that satisifes the constraints and is collision free could not be found."
                                        << "\nRequest was: \n"
      ...
      switch (error_code.val)
      {
        case moveit_msgs::MoveItErrorCodes::FAILURE:
          ROS_ERROR_STREAM_NAMED("FABRIK", "Service failed with with error code: FAILURE");
          break;
        case moveit_msgs::MoveItErrorCodes::NO_IK_SOLUTION:
          ROS_ERROR_STREAM_NAMED("FABRIK", "Service failed with with error code: NO IK SOLUTION");
          break;
        default:
          ROS_ERROR_STREAM_NAMED("FABRIK", "Service failed with with error code: " << error_code.val);
      }
      return false;
    }
  }
  ...
  {
    ROS_DEBUG_STREAM_NAMED("FABRIK", "Calling solution callback on IK solution");
    ...
    if (error_code.val != error_code.SUCCESS)
    {
      ...
      {
        case moveit_msgs::MoveItErrorCodes::FAILURE:
          ROS_ERROR_STREAM_NAMED("FABRIK", "IK solution callback failed with with error code: FAILURE");
          break;
        case moveit_msgs::MoveItErrorCodes::NO_IK_SOLUTION:
          ROS_ERROR_STREAM_NAMED("FABRIK", "IK solution callback failed with with error code: "
                                        "NO IK SOLUTION");
          break;
        default:
          ROS_ERROR_STREAM_NAMED("FABRIK", "IK solution callback failed with with error code: " << error_code.val);
      }
      return false;
    }
  }

  ROS_INFO_STREAM_NAMED("FABRIK", "IK Solver Succeeded!");
  return true;
}
                                        ...
                                        std::vector<geometry_msgs::Pose>& poses) const
{
  ...
  {
    ROS_ERROR_NAMED("FABRIK", "Joint angles vector must have size: %d", dimension_);
    return false;
  }

  ROS_ERROR_STREAM_NAMED("FABRIK", "Forward kinematics not implemented");
...
const std::vector<std::string>& FABRIKKinematicsPlugin::getVariableNames() const
{
